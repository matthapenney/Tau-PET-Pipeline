...
echo "STAGE 7: STEP 1 --> ${subj} REGISTER T1 SKULLSTRIP MASK TO PET SPACE"
cmd="${FSLFLIRT}
	-in ${MPRAGE}/${subj}_N4_brain.nii.gz \
	-ref ${PET_OUT}/${subj}_TAUPET_prelim_bet.nii.gz \
	-applyxfm \
...
eval ${cmd}
touch ${NOTE}
echo -e "STAGE 7: STEP 1 --> ${subj} REGISTER T1 SKULLSTRIP MASK TO PET SPACE \r\n" >> ${NOTE}
echo -e "COMMAND -> ${cmd}\r\n" >> ${NOTE}

# STEP (2) - THRESHOLD AND MULTIPLY MASK
echo "STAGE 7: STEP 2 --> ${subj} THRESHOLD AND BINARIZE T1 SKULLSTRIPPED MASK IN PET SPACE"
...
eval ${cmd}
touch ${NOTE}
echo -e "STAGE 7: STEP 2 --> ${subj} THRESHOLD AND BINARIZE T1 SKULLSTRIPPED MASK IN PET SPACE \r\n" >> ${NOTE}
echo -e "COMMAND -> ${cmd}\r\n" >> ${NOTE}


# STEP (3) - MULTIPLY BINARY MASK TO PET IMAGE TO GET PET SKULLSTRIPPED
echo "STAGE 7: STEP 3 --> ${subj} MULTIPLY MASK TO PET TO CREATE SKULLSTRIPPED PET"
...
eval ${cmd}
touch ${NOTE}
echo -e "STAGE 7: STEP 3 --> ${subj} MULTIPLY MASK TO PET TO CREATE SKULLSTRIPPED PET \r\n" >> ${NOTE}
echo -e "COMMAND -> ${cmd}\r\n" >> ${NOTE}


# STEP (4) - PET SKULL STRIP => Register T1 to PET TO MAKE PRETTY BRAIN PIC
echo "STAGE 7: STEP 4 --> ${subj} REGISTER T1 SKULLSTRIP TO PET SPACE"
cmd="${FSLFLIRT}
	-in ${MPRAGE}/${subj}_N4_brain.nii.gz \
...
if [[ -f "$N4" ]]; then
 eval ${cmd}
 touch ${NOTE}
 echo -e "STAGE 7: STEP 4 --> ${subj} REGISTER T1 SKULLSTRIP TO PET SPACE \r\n" >> ${NOTE}
 echo -e "COMMAND -> ${cmd}\r\n" >> ${NOTE}
else
 eval ${cmd2}
 touch ${NOTE}
