...
echo "STAGE: 1 STEP 1 --> ${subj} REGISTER MNI BRAIN to NATIVE T1 SPACE" 
cmd="${FSLFLIRT} \
	 ...
	 -ref ${MPRAGE}/${subj}_N4_brain.nii.gz \
 	 ...
 	 -datatype float"
eval ${cmd}
echo -e "STAGE: 1 STEP 1 --> ${subj} REGISTER MNI to NATIVE T1 SPACE \r\n" >> ${NOTE}
echo -e "COMMAND -> ${cmd}\r\n" >> ${NOTE}

#STEP (2)- APPLY TRANSFORMATION TO CEREBELLUM LABEL MAP 
#USING NEAREST NEIGHBOUR BECAUSE IT IS A LABEL MAP, NOT! LINEAR
echo "STAGE: 1 STEP 2 --> ${subj} APPLY TRANSFORMATION TO CEREBELLUM MASK"
cmd="${FSLFLIRT} \
 	...
 	-datatype float"
eval ${cmd}
echo -e "STAGE: 1 STEP 2 --> ${subj} APPLY TRANSFORMATION TO MNI CEREBELLUM MASK \r\n" >> ${NOTE}
echo -e "COMMAND -> ${cmd}\r\n" >> ${NOTE}
