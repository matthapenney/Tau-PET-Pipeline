...
#STAGE 5 - EXTRACT CEREBELLUM IN NATIVE SPACE & BINARIZE

#------------------------------------------------------------------

# STEP (1) EXTRACT REFERENCE REGION FROM CEREBELLUM
echo "Stage 5: Step 1 --> ${subj} ${rr} MNI reference region extracted."

cmd="${REGIONEXTRACT} ${T1}/${subj}_cereMNI_to_native.nii.gz ${RRLABEL} ${PET_OUT}/${subj}_cereMNI_native_ref_${rr}.nii.gz"
eval ${cmd}
touch ${NOTE}
echo -e "[[Stage 5: Step 1]]\r\n Command --> ${cmd}\r\n" >> ${NOTE}


# STEP (2) BINARIZE EXTRACTED CEREBELLUM REGION
echo "Stage 5: Step 3 --> ${subj} ${rr} binarized."
cmd="${FSLMATHS} ${PET_OUT}/${subj}_cereMNI_native_ref_${rr}.nii.gz -bin ${PET_OUT}/${subj}_cereMNI_native_ref_${rr}_bin.nii.gz"
eval ${cmd}
touch ${NOTE}
echo -e "[[Stage 5: Step 2]]\r\n Command --> ${cmd}\r\n" >> ${NOTE}
